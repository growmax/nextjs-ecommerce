...
interface CatalogCodeFilterProps {
...
/**
 * CatalogCodeFilter Component
 * Filter by catalog codes
 */
export function CatalogCodeFilter({
  options,
  selectedCodes,
  onToggle,
  isLoading = false,
}: CatalogCodeFilterProps) {
          ...
          {filteredOptions.length === 0 ? (
            ...
            filteredOptions.map((option) => {
              ...
              return (
                ...
                >
                  <Checkbox
                    id={`catalog-${option.value}`}
                    checked={isSelected}
                    onCheckedChange={() => onToggle(option.value)}
                    className="shrink-0"
                  />
                  <Label
                    htmlFor={`catalog-${option.value}`}
