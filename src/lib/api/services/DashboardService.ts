...
export class DashboardService extends BaseService<DashboardService> {
  ...
  transformOrderDataForChart(data: DashboardApiResponse) {
    ...
    return sortedEntries.map(([dateValue, values]) => {
      ...
      return {
        month: `${fullMonthName} ${year}`, // Full month name with year for display
        monthAbbrev, // Keep abbreviated version
        fullMonth: dateValue, // Keep original dateValue
        dateValue, // Original dateValue from API
