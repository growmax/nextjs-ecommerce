...
/**
 * Normalize domain for cache key generation
 * Removes protocol, trailing slashes, and converts to lowercase
 */
function normalizeDomainForCache(domain: string): string {
  return domain
    .toLowerCase()
...
function getHomepageCacheKey(domain: string, tenantCode?: string): string {
  ...
  const sanitizedTenant = tenant
    ...
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
  return `homepage:config:${normalizedDomain}:${sanitizedTenant}`;
}
...
export async function getHomepageConfig(
  ...
  try {
    ...
    // Generate cache key
    const cacheKey = getHomepageCacheKey(domain, tenantCode);
    const ttl = 3600; // 1 hour TTL to match page revalidation

    // Wrap the storefront config fetch with Redis cache
    const response = await withRedisCache(
      cacheKey,
      () => service.getStoreFrontConfig(queryDomain, context),
      ttl
    );
