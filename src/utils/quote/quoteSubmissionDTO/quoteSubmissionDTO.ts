...
export const prepareQuoteSubmissionDTO = (
  ...
  if (overViewValues?.quoteDivisionId) {
    body.quoteDivisionId =
      typeof overViewValues.quoteDivisionId === "object"
  ...
  if (overViewValues?.orderType) {
    body.orderTypeId =
      typeof overViewValues.orderType === "object"
  ...
  if (values.branchBusinessUnit) {
    body.branchBusinessUnitId =
      typeof values.branchBusinessUnit === "object"
  ...
  if (values.buyerCurrencyId) {
    ...
    body.buyerCurrencyId =
      typeof values.buyerCurrencyId === "object"
  ...
  if (body.cartValue) {
    body.subTotal =
      body?.VDapplied && body?.VDDetails?.subTotal
        ? body.VDDetails.subTotal
        : body.cartValue.totalValue;

    body.overallTax =
      body?.VDapplied && body?.VDDetails?.overallTax
    ...
    body.calculatedTotal =
      body?.VDapplied && body?.VDDetails?.calculatedTotal
        ? body.VDDetails.calculatedTotal
        : body.cartValue.calculatedTotal;

    body.roundingAdjustment =
      body?.VDapplied && body?.VDDetails?.roundingAdjustment
  ...
  if (body.dbProductDetails && Array.isArray(body.dbProductDetails)) {
    body.dbProductDetails = map(body.dbProductDetails, (prod: any) => {
      const formattedProd: any = { ...prod };

      // Account Owner
      formattedProd.accountOwnerId = prod.accountOwner
        ? typeof prod.accountOwner === "object"
          ? parseInt(prod.accountOwner.id)
          : parseInt(prod.accountOwner)
        : null;
      ...
      // Warehouse
      formattedProd.orderWareHouseId =
        prod.wareHouse?.id || prod.orderWareHouseId;
      formattedProd.orderWareHouseName =
        prod.wareHouse?.wareHouseName || prod.orderWareHouseName;

      // P&F values
      formattedProd.pfPercentage =
        body.quoteTerms?.pfPercentage ||
        body.pfRate ||
        prod.pfPercentage ||
        null;
      formattedProd.pfValue =
        body.quoteTerms?.pfValue || body.pfRate || prod.pfValue || null;
 ...
 */
export function checkIsBundleProduct(bundleProducts?: any[]): boolean {
  if (
