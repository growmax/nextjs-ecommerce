...
jest.mock("@/utils/calculation/salesCalculation/salesCalculation", () => ({
  getAccounting: jest.fn((currency, value) => `₹${value}`),
}));
...
describe("QuoteSummaryContent", () => {
  ...
  describe("Validation", () => {
    it("should show error when products have negative prices", async () => {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const useSummaryDefaultModule = require("@/hooks/summary/useSummaryDefault");
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      jest
    ...
    it("should show error when addresses are missing", async () => {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const useSummaryDefaultModule = require("@/hooks/summary/useSummaryDefault");
      jest.spyOn(useSummaryDefaultModule, "default").mockReturnValueOnce({
        initialValues: {
          ...mockInitialValues,
  ...
  describe("Loading States", () => {
    it("should show loading skeleton when data is loading", async () => {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const useSummaryDefaultModule = require("@/hooks/summary/useSummaryDefault");
      jest.spyOn(useSummaryDefaultModule, "default").mockReturnValue({
        initialValues: mockInitialValues,
        isLoading: true,
      });
