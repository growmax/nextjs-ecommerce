...
export default function useSummaryForm<T extends Record<string, any>>(
  ...
  const handleFormErrors = useCallback((
    errors: any,
    getValuesFn: UseFormReturn<T>["getValues"]
  ) => {
    ...
    if (errors.buyerReferenceNumber) {
      scrollToErrorField("endCustomerInfo");
      toast.error(errors.buyerReferenceNumber?.message || "Invalid reference number");
      return;
    }
    ...
    if (errors.sprDetails) {
      ...
      if (sprErrors.priceJustification) {
        toast.error(sprErrors.priceJustification?.message || "Invalid price justification");
        return;
      }
      if (sprErrors.competitorNames) {
        toast.error(sprErrors.competitorNames?.message || "Invalid competitor names");
        return;
      }
    }
